...
static void sm90_m_grouped_bf16_gemm_contiguous(const torch::Tensor& a,
    ...
    const auto code = SM90BF16GemmRuntime::generate(args);
    const auto key = PrecompiledKey::from(desc, config, "bf16_gemm");
    const auto runtime = compiler->build("sm90_m_grouped_bf16_gemm_contiguous", code, key);
    SM90BF16GemmRuntime::launch(runtime, args);
}
...
static void sm90_bf16_m_grouped_gemm_masked(const torch::Tensor& a,
    ...
    const auto code = SM90BF16GemmRuntime::generate(args);
    const auto key = PrecompiledKey::from(desc, config, "bf16_gemm");
    const auto runtime = compiler->build("sm90_bf16_m_grouped_gemm_masked", code, key);
    SM90BF16GemmRuntime::launch(runtime, args);
}
...
static void sm90_bf16_k_grouped_gemm(const torch::Tensor& a,
    ...
    const auto code = SM90BF16GemmRuntime::generate(args);
    const auto key = PrecompiledKey::from(desc, config, "bf16_gemm");
    const auto runtime = compiler->build("sm90_bf16_k_grouped_gemm", code, key);
    SM90BF16GemmRuntime::launch(runtime, args);
}
...
static void sm90_bf16_bhr_hdr_bhd(const torch::Tensor& tensor_a,
    ...
    const auto code = SM90BF16GemmRuntime::generate(args);
    const auto key = PrecompiledKey::from(desc, config, "bf16_gemm");
    const auto runtime = compiler->build("sm90_bf16_bhr_hdr_bhd", code, key);
    SM90BF16GemmRuntime::launch(runtime, args);
}
...
static void sm90_bf16_bhd_hdr_bhr(const torch::Tensor& tensor_a,
    ...
    const auto code = SM90BF16GemmRuntime::generate(args);
    const auto key = PrecompiledKey::from(desc, config, "bf16_gemm");
    const auto runtime = compiler->build("sm90_bf16_bhd_hdr_bhr", code, key);
    SM90BF16GemmRuntime::launch(runtime, args);
}
